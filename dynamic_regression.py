...
class DynamicRegression:
    ...
    def set_options(self, options: dict = None) -> None:
        ...
        if options is None:
            ...
            return
        self.sigma2_concentration = self._to_float(options["sigma2_concentration"])
        self.sigma2_rate = self._to_float(options["sigma2_rate"])
        self.tau2_concentration = self._to_float(options["tau2_concentration"])
        self.tau2_rate = self._to_float(options["tau2_rate"])
...
class DynamicRegressionPlotter:
    ...
    def timeseries_regression_coeffs(
        ...
        p += theme(legend_position=(0.8, 0.9))
        return p
